...
| Create task tab | `zellij action new-tab --cwd <dir> --name <scoped-title>` | Normally returns the created tab's bare integer id on stdout, but 0.44.1 can create a task tab after the prior one closes while returning empty stdout. The adapter accepts that empty form only after a bounded five-second inventory poll finds exactly one matching scoped title and one non-plugin pane; a normal numeric tab id retains the broader delayed-pane wait. No `--no-focus`-equivalent flag exists at all - see "Focus-steal on new-tab" below. The caller passes `fm-<id>`, but the adapter creates `fm-<home-label>-<id>`. |
...
2. Output-**shape** validation rejects the "session not found" text fallback structurally: `fm_backend_zellij_create_task` accepts `new-tab`'s bare integer stdout or recovers exactly one matching scoped title from JSON when stdout is empty, and every `list-panes`/`list-tabs` consumer pipes through `jq`, which fails to parse the plain-text fallback as JSON.

### Empty `new-tab` output after tab recreation
