...
| Authored-source and package provenance closure | `SOURCE_PROVENANCE.json`, `bin/agent-os-source-context.sh`, `tests/agent-os-source-provenance.test.sh`, OCI labels, and pipeline commit/tree/archive evidence preserve exact source ancestry while excluding credentials and operational state | Source contract proven on the integration branch; merge and publication remain separate approvals |
