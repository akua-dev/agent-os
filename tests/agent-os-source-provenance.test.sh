...
for path in \
  ...
  grep -Fx "!$path" "$ROOT/.dockerignore" >/dev/null \
    || fail "local Docker context must re-include tracked harness integration: $path"
done

for harness_home in .pi .codex .claude .grok .opencode; do
  grep -Fx "$harness_home/**" "$ROOT/.dockerignore" >/dev/null \
    || fail "Docker context must default-deny untracked harness-home content: $harness_home"
  ! grep -Fx "$harness_home" "$ROOT/.dockerignore" >/dev/null \
    || fail "Docker context must not hide the entire tracked harness integration tree: $harness_home"
done
